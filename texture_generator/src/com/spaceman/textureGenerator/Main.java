...
public class Main {
    ...
    private static ArrayList<Model> collectDynmapModels() {
        ...
        try {
            Field builtin_icons_field = MarkerAPIImpl.class.getDeclaredField("builtin_icons");
            ...
            for (String markerName : markerArray) {
                ...
                InputStream in = MarkerAPIImpl.class.getResourceAsStream("/markers/" + markerName + ".png");
                
                java.nio.file.Files.copy(in, outputFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
                in.close();
    ...
    private static JsonObject getJsonObject(Material material) {
        ...
        if (j == null) {
            File jsonFile = new File(moduleName + "/src/model_json/" + material.name().toLowerCase() + ".json");
    ...
    private static void copyTextures(ArrayList<Model> models, String packDir) {
        ...
        for (Model model : models) {
            File texture;
            texture = new File(moduleName + "/src/icons/" + packDir + "/" + model.name + ".png");
            if (texture.exists()) {
                try {
                    if (model.subDir == null) {
...
//                throw new IllegalArgumentException("Texture '" + texture.getName() + "' does not exist");
        ...
        File packPNG = new File(moduleName + "/src/icons/" + packDir + "/pack.png");
        if (packPNG.exists()) {
            try {
